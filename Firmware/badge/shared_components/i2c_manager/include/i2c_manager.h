...
// Generic bus aliases
#define I2C_BUS_A I2C_NUM_0
#define I2C_BUS_B I2C_NUM_1

typedef struct {
    uint8_t bus_index; // 0 or 1
    int8_t sda_pin;    // -1 to skip
    int8_t scl_pin;    // -1 to skip
    bool enable_internal_pullups;
    uint32_t default_speed_hz; // 0 => use per-device speed only
    bool enabled;
} i2c_manager_pin_config_t;

typedef enum {
    I2C_MUX_NONE = 0,
    I2C_MUX_TCA9544A, // 4-channel
    I2C_MUX_PCA9548A, // 8-channel
} i2c_mux_type_t;

typedef struct {
    i2c_mux_type_t type;
    uint8_t address;         // 7-bit mux address
    uint8_t channels;        // number of channels supported
    uint8_t current_channel; // cached selection (0xFF unknown)
    bool present;            // probed ok
    int8_t int_gpio;         // optional INT GPIO (-1 if unused)
} i2c_manager_mux_t;

// I2C bus info structure
typedef struct {
    bool initialized;
    i2c_manager_pin_config_t pin_config;
    i2c_master_bus_config_t config;
    i2c_master_bus_handle_t handle;
    SemaphoreHandle_t lock;
    i2c_manager_mux_t mux; // optional mux if enabled... type==NONE if unused
} i2c_manager_bus_t;

// I2C device config tracking structure
typedef struct {
    uint8_t bus_index; // I2C bus index
    uint8_t channel;   // 0xFF means no mux channel
    i2c_device_config_t config;
    esp_log_level_t log_level;
} i2c_manager_device_config_t;

typedef struct {
    int8_t bus_index;      // -1 any
    uint16_t channel_mask; // bit mask; 0 => all channels (if mux present)
    uint8_t address;       // 0 any
...
typedef struct {
    ...
    uint8_t channel;
    uint16_t address;
    i2c_manager_device_config_t config;
...
// -------------------------------------------------------------------------------------------------
// Interrupt Aggregation (only if supported by the mux such as TCA9544A)
// -------------------------------------------------------------------------------------------------
typedef void (*i2c_manager_mux_int_cb_t)(uint8_t bus_index, uint8_t channel, uint32_t raw, void *ctx);
...
/**
 * @brief Check if I2C reserved addresses are allowed
 *
 * @return bool True if reserved addresses are allowed, false otherwise
 */
bool i2c_manager_reserved_allowed(void);

/**
 * @brief Set whether I2C reserved addresses are allowed
 *
 * @param[in] allow True to allow reserved addresses, false to disallow
 */
 ...
 *    This function will add the device to the list of known devices and add it to the I2C bus. If the
 *    device already exists, it will check to see if anything is different and if so, it will remove and
 *    re-add the device with the new configuration.
 *
 * @param[in] device Device configuration
