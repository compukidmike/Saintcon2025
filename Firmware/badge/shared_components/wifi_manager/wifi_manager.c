...
// Provisioned WiFi creds stored in the secure element
static bool provisioned_creds_loaded                    = false;
static bool provisioned_creds_present                   = false;
static char se_ssid[SLOT_CAP(SE_KEY_WIFI_SSID)]         = {0};
static char se_password[SLOT_CAP(SE_KEY_WIFI_PASSWORD)] = {0};

static void load_provisioned_creds() {
    if (provisioned_creds_loaded) {
        return;
    }
    provisioned_creds_loaded = true;

    uint8_t ssid_slot[SLOT_CAP(SE_KEY_WIFI_SSID)]     = {0};
    uint8_t pass_slot[SLOT_CAP(SE_KEY_WIFI_PASSWORD)] = {0};

    if (se_read_slot(SE_KEY_WIFI_SSID, ssid_slot, sizeof(ssid_slot)) == ESP_OK && se_slot_touched(SE_KEY_WIFI_SSID) &&
        se_read_slot(SE_KEY_WIFI_PASSWORD, pass_slot, sizeof(pass_slot)) == ESP_OK && se_slot_touched(SE_KEY_WIFI_PASSWORD)) {
        memcpy(se_ssid, ssid_slot, sizeof(ssid_slot));
        se_ssid[sizeof(se_ssid) - 1] = '\0';
        memcpy(se_password, pass_slot, sizeof(pass_slot));
        se_password[sizeof(se_password) - 1] = '\0';
        provisioned_creds_present            = true;
        ESP_LOGI(TAG, "Provisioned WiFi creds found (SSID len=%u)", (unsigned)strnlen(se_ssid, sizeof(se_ssid)));
    } else {
        ESP_LOGD(TAG, "No provisioned WiFi creds found");
    }
}
...
void wifi_force_reconnect() {
    ESP_LOGI(TAG, "Force WiFi reconnect requested");
    // Reset load flag so next loop picks up any newly written creds
    provisioned_creds_loaded = false;
...
esp_err_t init_wifi_sta() {
    ...
    // For now disable WiFi power management to prevent some beacon timeouts and disconnections I have noticed
    ESP_RETURN_ON_ERROR(esp_wifi_set_ps(WIFI_PS_NONE), TAG, "Failed to disable WiFi power save");
...
esp_err_t try_connect(const char *ssid, const char *password) {
    ...
    // For now prefer WPA2 over WPA3 and make PMF optional due to some finnicky behavior I've noticed
    ...
    if (event_base == WIFI_EVENT) {
        switch (event_id) {
            ...
            case WIFI_EVENT_STA_DISCONNECTED: {
                ...
                ESP_LOGW(TAG, "WiFi disconnected from %s, reason: %d", event->ssid, event->reason);
...
void wifi_manager_task(void *_arg) {
    ...
    for (;;) {
        load_provisioned_creds();

        // If provisioned creds exist, use them exclusively since they are for the real conference network
        if (provisioned_creds_present) {
            if (try_connect(se_ssid, se_password) != ESP_OK) {
                ESP_LOGW(TAG, "Failed to connect with provisioned conference credentials");
            } else {
                delay = initialDelay;
            }
        } else if (try_connect(CONFIG_CONFERENCE_WIFI_SSID, CONFIG_CONFERENCE_WIFI_PASSWORD) != ESP_OK) {
#ifdef CONFIG_ALLOW_EXTERNAL_WIFI_NETWORKS
...
esp_err_t wifi_manager_init() {
    ...
    // Initialize the event loop if not already created
    ...
    if (event_loop_result != ESP_OK && event_loop_result != ESP_ERR_INVALID_STATE) {
        ESP_LOGE(TAG, "Failed to create event loop: %s", esp_err_to_name(event_loop_result));
