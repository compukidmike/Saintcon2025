...
class ApiClient {
  public:
    ...
    // Badge API endpoints
    ApiResult redeemCode(const std::string_view code);
    ApiResult registerBadge(const std::string_view player_name);
    ApiResult getStatus();
    ApiResult getMessages();
    ApiResult markMessageRead(const uint32_t message_id);
    ApiResult listFiles();
    ApiResult downloadFile(const std::string_view file_sha256, const std::string_view file_path);
    ApiResult konamiEntered();
    ApiResult dawnAccord(bool ai_inhibit, bool ai_security, std::string_view reason);
    ApiResult getAchievements();
    ApiResult ctfHintsPassword();
    ...
    // Game API endpoints
    ApiResult playerStatus();
    ApiResult requestCreditSwap(const uint32_t amount);
    ApiResult creditSwapStatus(const std::string_view code);
    ApiResult cancelCreditSwap(const std::string_view code);
    ApiResult getTopologyData();
