...
#define SHA256_CHUNK_SIZE (8 * 1024)  // 8 KB chunks for SHA256 verification
...
const std::map<ApiClient::EndpointKey, ApiClient::AuthLevel> ApiClient::api_metadata = {
    {{"/api/badge/challenge",                   HTTP_METHOD_POST},   ApiClient::AuthLevel::SIGNED},
    {{"/api/badge/provision",                   HTTP_METHOD_POST},   ApiClient::AuthLevel::BASIC},
    {{"/api/badge/provision/verify",            HTTP_METHOD_POST},   ApiClient::AuthLevel::SIGNED},
    {{"/api/badge/provision/wireless-psk",      HTTP_METHOD_POST},   ApiClient::AuthLevel::SIGNED},
    {{"/api/badge/redeem_code",                 HTTP_METHOD_POST},   ApiClient::AuthLevel::CHALLENGE},
    {{"/api/badge/register",                    HTTP_METHOD_POST},   ApiClient::AuthLevel::CHALLENGE},
    {{"/api/badge/dawn-accord",                 HTTP_METHOD_POST},   ApiClient::AuthLevel::CHALLENGE},
    {{"/api/badge/status",                      HTTP_METHOD_GET},    ApiClient::AuthLevel::CHALLENGE},
    {{"/api/badge/request_credit_swap",         HTTP_METHOD_POST},   ApiClient::AuthLevel::CHALLENGE},
    {{"/api/badge/credit_swap_status/<code>",   HTTP_METHOD_GET},    ApiClient::AuthLevel::CHALLENGE},
    {{"/api/badge/cancel_credit_swap/<code>",   HTTP_METHOD_DELETE}, ApiClient::AuthLevel::CHALLENGE},
    {{"/api/badge/messages",                    HTTP_METHOD_GET},    ApiClient::AuthLevel::CHALLENGE},
    {{"/api/badge/message/read",                HTTP_METHOD_POST},   ApiClient::AuthLevel::CHALLENGE},
    {{"/api/badge/files/list",                  HTTP_METHOD_GET},    ApiClient::AuthLevel::BASIC},
    {{"/api/badge/files/download/<sha256>",     HTTP_METHOD_GET},    ApiClient::AuthLevel::CHALLENGE},
    {{"/api/badge/konami",                      HTTP_METHOD_POST},   ApiClient::AuthLevel::CHALLENGE},
    {{"/api/badge/achievements",                HTTP_METHOD_GET},    ApiClient::AuthLevel::CHALLENGE},
    {{"/api/badge/ctf-hints-password",          HTTP_METHOD_GET},    ApiClient::AuthLevel::CHALLENGE},
    {{"/api/firmware/version",                  HTTP_METHOD_GET},    ApiClient::AuthLevel::SIGNED},
    {{"/api/firmware/badge",                    HTTP_METHOD_GET},    ApiClient::AuthLevel::SIGNED},
    {{"/api/firmware/badge",                    HTTP_METHOD_HEAD},   ApiClient::AuthLevel::NONE},
    {{"/topology/data",                         HTTP_METHOD_GET},    ApiClient::AuthLevel::NONE},
};
...
/**
 * @brief Compute SHA256 hash of a file in chunks to avoid loading entire file into memory
 *
 * @param filepath Path to the file to hash
 * @param hash_output Buffer to store the resulting hash (must be at least 32 bytes)
 * @return esp_err_t ESP_OK on success, ESP_FAIL on error
 */
static esp_err_t compute_file_sha256(const char *filepath, uint8_t *hash_output) {
    FILE *file = fopen(filepath, "rb");
    if (!file) {
        ESP_LOGE(TAG, "Failed to open file for SHA256 verification: %s", filepath);
        return ESP_FAIL;
    }
    ...
    if (ret != 0) {
        ESP_LOGE(TAG, "Failed to start SHA256: %d", ret);
    ...
    while ((bytes_read = fread(buffer, 1, sizeof(buffer), file)) > 0) {
        ret = mbedtls_sha256_update(&ctx, buffer, bytes_read);
        if (ret != 0) {
            ESP_LOGE(TAG, "Failed to update SHA256: %d", ret);
            mbedtls_sha256_free(&ctx);
            fclose(file);
            return ESP_FAIL;
        }
    ...
    if (ret != 0) {
        ESP_LOGE(TAG, "Failed to finish SHA256: %d", ret);
        return ESP_FAIL;
    }
    ...
    switch (evt->event_id) {
        ...
        case HTTP_EVENT_ON_DATA: //
            ...
            if (!esp_http_client_is_chunked_response(evt->client)) {
                // If we're streaming to a file, write directly to SPIFFS
                if (client->output_file != nullptr) {
                    size_t written = fwrite(evt->data, 1, evt->data_len, client->output_file);
                    if (written != evt->data_len) {
                        ESP_LOGE(TAG, "Failed to write %d bytes to file, only wrote %zu", evt->data_len, written);
                        return ESP_FAIL;
                    }
                    client->bytes_written += written;
                    ESP_LOGD(TAG, "Wrote %zu bytes to file (total: %zu)", written, client->bytes_written);
                } else {
                    // Otherwise, buffer the response in memory
                    if (client->log_data) {
...
esp_err_t ApiClient::applyAuthHeaders(esp_http_client_handle_t client, const std::string_view method, const std::string_view path,
    ...
    if (auth_level == AuthLevel::CHALLENGE) {
        ...
        ApiResult challenge_response = doRequest("/api/badge/challenge", "POST");
...
ApiClient::ApiResult ApiClient::doRequest(const std::string_view endpoint, const std::string_view method,
                                          const std::string_view payload, const std::string_view output_file_path) {
    // Look up the method from the given method string
    esp_http_client_method_t http_method = HTTP_METHOD_GET;
    if (method_names.right.find(std::string(method)) != method_names.right.end()) {
        http_method = method_names.right.at(std::string(method));
    ...
    // Open output file if streaming to SPIFFS
    if (!output_file_path.empty()) {
        context.output_file = fopen(output_file_path.data(), "wb");
        if (context.output_file == nullptr) {
            ESP_LOGE(TAG, "Failed to open file for writing: %s", output_file_path.data());
            return ApiError::GENERIC_ERROR;
        }
        ESP_LOGI(TAG, "Opened file for streaming: %s", output_file_path.data());
    }
    ...
    if (esp_err_t err = esp_http_client_perform(client); err != ESP_OK) {
        ...
        if (context.output_file != nullptr) {
            ...
            if (!output_file_path.empty()) {
                remove(output_file_path.data());
                ESP_LOGW(TAG, "Deleted incomplete file: %s", output_file_path.data());
            }
        }
        ...
        // Log appropriate metrics based on whether we're streaming to a file
        if (context.output_file != nullptr) {
            ESP_LOGD(TAG, "HTTP Status = %d, content_length = %d, duration = %lldms, bytes_written = %zu", response.status_code,
                     content_length, duration_ms, context.bytes_written);
        } else {
        ...
        if (response.status_code >= 400) {
            ...
            // Clean up file on HTTP error
            if (context.output_file != nullptr) {
                fclose(context.output_file);
                context.output_file = nullptr;
                if (!output_file_path.empty()) {
                    remove(output_file_path.data());
                    ESP_LOGW(TAG, "Deleted file due to HTTP error: %s", output_file_path.data());
                }
            }
        }
    }

    // Close output file if we were streaming
    if (context.output_file != nullptr) {
        fclose(context.output_file);
        ESP_LOGI(TAG, "Closed output file: %s (%zu bytes written)", output_file_path.data(), context.bytes_written);
    }
...
ApiClient::ApiResult ApiClient::provisionSecrets() {
    return doRequest("/api/badge/provision", "POST");
}

ApiClient::ApiResult ApiClient::provisionWirelessPSK() {
    return doRequest("/api/badge/provision/wireless-psk", "POST");
}

ApiClient::ApiResult ApiClient::provisionVerify() {
    return doRequest("/api/badge/provision/verify", "POST");
}
...
ApiClient::ApiResult ApiClient::redeemCode(const std::string_view code) {
    // Pre-check: it should be a 5-character base-36 code
    char base_chars[] = "ABCDEFGHJKLMNPQRSTUVWXYZ23456789";
    if (code.length() != 5) {
        ESP_LOGE(TAG, "Invalid code length");
        return ApiError::REQUEST_VALIDATION_ERROR;
    }
    for (char c : code) {
        if (std::find(std::begin(base_chars), std::end(base_chars), c) == std::end(base_chars)) {
            ESP_LOGE(TAG, "Invalid code character: %c", c);
            return ApiError::REQUEST_VALIDATION_ERROR;
        }
    }

    json payload = {{"code", code}};
    return doRequest("/api/badge/redeem_code", "POST", payload.dump());
}

ApiClient::ApiResult ApiClient::registerBadge(const std::string_view player_name) {
    json payload = {{"player_name", player_name}};
    return doRequest("/api/badge/register", "POST", payload.dump());
}

ApiClient::ApiResult ApiClient::getStatus() {
    return doRequest("/api/badge/status", "GET");
}

ApiClient::ApiResult ApiClient::getMessages() {
    return doRequest("/api/badge/messages", "GET");
}

ApiClient::ApiResult ApiClient::markMessageRead(const uint32_t message_id) {
    json payload = {{"message_id", message_id}};
    return doRequest("/api/badge/messages/read", "POST", payload.dump());
}

ApiClient::ApiResult ApiClient::listFiles() {
    return doRequest("/api/badge/files/list", "GET");
}

ApiClient::ApiResult ApiClient::downloadFile(const std::string_view file_sha256, const std::string_view file_path) {
    // Make sure it's a valid SHA256 hex string
    if (file_sha256.length() != 64) {
        ESP_LOGE(TAG, "Invalid SHA256 length: %zu (expected 64)", file_sha256.length());
        return ApiError::REQUEST_VALIDATION_ERROR;
    }
    for (char c : file_sha256) {
        if (!std::isxdigit(c)) {
            ESP_LOGE(TAG, "Invalid SHA256 character: %c", c);
            return ApiError::REQUEST_VALIDATION_ERROR;
        }
    }

    // Convert SHA256 to lowercase for comparison
    std::string sha256_lower(file_sha256);
    std::transform(sha256_lower.begin(), sha256_lower.end(), sha256_lower.begin(),
                   [](unsigned char c) { return std::tolower(c); });

    // Create temporary file path using SHA256 as filename with .tmp prefix
    std::string temp_path = std::format("/spiffs/.tmp_{}", sha256_lower);

    ESP_LOGI(TAG, "Downloading file with SHA256: %s", sha256_lower.c_str());
    ESP_LOGI(TAG, "Temporary path: %s", temp_path.c_str());
    ESP_LOGI(TAG, "Final path: %s", file_path.data());

    // Download to temporary file first
    ApiResult result = doRequest(std::format("/api/badge/files/download/{}", sha256_lower), "GET", "", temp_path);

    // Check if download failed
    if (std::holds_alternative<ApiError>(result)) {
        ESP_LOGE(TAG, "Download failed, cleaning up temporary file");
        unlink(temp_path.c_str());
        return result;
    }

    // Verify the SHA256 checksum of the downloaded file
    ESP_LOGI(TAG, "Verifying SHA256 checksum...");
    uint8_t computed_hash[32];
    if (compute_file_sha256(temp_path.c_str(), computed_hash) != ESP_OK) {
        ESP_LOGE(TAG, "Failed to compute SHA256 of downloaded file");
        unlink(temp_path.c_str());
        return ApiError::GENERIC_ERROR;
    }

    // Convert computed hash to hex string for comparison
    std::string computed_hash_hex = ApiHelpers::bytes_hex(std::vector<uint8_t>(computed_hash, computed_hash + 32), true);

    // Compare computed hash with expected hash
    if (computed_hash_hex != sha256_lower) {
        ESP_LOGE(TAG, "SHA256 verification failed!");
        ESP_LOGE(TAG, "Expected: %s", sha256_lower.c_str());
        ESP_LOGE(TAG, "Computed: %s", computed_hash_hex.c_str());
        unlink(temp_path.c_str());
        return ApiError::GENERIC_ERROR;
    }
    ...
    if (rename(temp_path.c_str(), file_path.data()) != 0) {
        ESP_LOGE(TAG, "Failed to rename %s to %s: %s", temp_path.c_str(), file_path.data(), strerror(errno));
        unlink(temp_path.c_str());
        return ApiError::GENERIC_ERROR;
    }

    ESP_LOGI(TAG, "File successfully downloaded and verified: %s", file_path.data());
    return result;
}

ApiClient::ApiResult ApiClient::konamiEntered() {
    json payload = {{"success", true}};
    return doRequest("/api/badge/konami", "POST", payload.dump());
}

ApiClient::ApiResult ApiClient::dawnAccord(bool ai_inhibit, bool ai_security, std::string_view reason) {
    json payload = {{"ai_inhibitor_enabled", ai_inhibit}, {"ai_security_strip_enabled", ai_security}};
    if (!reason.empty()) {
        payload["reason"] = reason;
    }
    return doRequest("/api/badge/dawn-accord", "POST", payload.dump());
}

ApiClient::ApiResult ApiClient::getAchievements() {
    return doRequest("/api/badge/achievements", "GET");
}

ApiClient::ApiResult ApiClient::ctfHintsPassword() {
    return doRequest("/api/badge/ctf-hints-password", "GET");
}
...
ApiClient::ApiResult ApiClient::getFirmwareHead() {
    return doRequest("/api/firmware/badge", "HEAD");
}
...
api_err_t ApiClient::doFirmwareUpdate() {
    ...
    // Set up the HTTP client configuration
    esp_http_client_config_t config = {};
    std::string url                 = std::string(API_BASE_URL) + "/api/firmware/badge";
    config.url                      = url.c_str();
    ...
    ota_config.http_client_init_cb    = [](esp_http_client_handle_t client) -> esp_err_t {
        ...
        ret = self->applyAuthHeaders(client, "GET", "/api/firmware/badge");
        if (ret != ESP_OK) {
...
ApiClient::ApiResult ApiClient::playerStatus() {
    return doRequest("/api/badge/status", "GET");
}

ApiClient::ApiResult ApiClient::requestCreditSwap(const uint32_t amount) {
    json payload = {{"credit_amount", amount}};
    return doRequest("/api/badge/request_credit_swap", "POST", payload.dump());
}

ApiClient::ApiResult ApiClient::creditSwapStatus(const std::string_view code) {
    return doRequest(std::format("/api/badge/credit_swap_status/{}", code), "GET");
}

ApiClient::ApiResult ApiClient::cancelCreditSwap(const std::string_view code) {
    return doRequest(std::format("/api/badge/cancel_credit_swap/{}", code), "DELETE");
}

ApiClient::ApiResult ApiClient::getTopologyData() {
    return doRequest("/topology/data", "GET");
}
