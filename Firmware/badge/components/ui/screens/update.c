...
lv_obj_t *create_update_screen() {
    ...
    lv_style_set_border_color(&progress_style_bg, lv_color_hex(RED_DIM));
    lv_style_set_border_width(&progress_style_bg, 3);
    lv_style_set_pad_all(&progress_style_bg, 6);
    lv_style_set_radius(&progress_style_bg, 7);

    static lv_style_t progress_style_indicator;
    lv_style_init(&progress_style_indicator);
    lv_style_set_bg_color(&progress_style_indicator, lv_color_hex(RED_LIGHT));
    lv_style_set_bg_opa(&progress_style_indicator, LV_OPA_COVER);
    lv_style_set_radius(&progress_style_indicator, 3);
    ...
    lv_obj_set_size(update_progress, LV_HOR_RES - 40, 25);
    ...
    // Label above to show the version being downloaded
    version_label = lv_label_create(scr_update);
    lv_label_set_text(version_label, "Version: 0.0.0");
    lv_obj_set_style_text_font(version_label, &lv_font_montserrat_16, LV_PART_MAIN);
    lv_obj_set_style_text_color(version_label, lv_color_hex(GRAY_TINT_3), LV_PART_MAIN);
    lv_obj_set_style_text_align(version_label, LV_TEXT_ALIGN_CENTER, LV_PART_MAIN);
    lv_obj_align_to(version_label, update_progress, LV_ALIGN_OUT_TOP_MID, 0, -10);

    // Label below to show bytes and percentage
    progress_label = lv_label_create(scr_update);
    lv_label_set_text(progress_label, "0%\n(0/0)");
    lv_obj_set_width(progress_label, LV_HOR_RES - 10);
    lv_obj_set_style_text_font(progress_label, &lv_font_montserrat_16, LV_PART_MAIN);
    ...
    lv_obj_set_style_text_line_space(progress_label, 4, LV_PART_MAIN);
    lv_obj_align_to(progress_label, update_progress, LV_ALIGN_OUT_BOTTOM_MID, 0, 10);

    // Create a message label
    update_message = lv_label_create(scr_update);
    lv_label_set_text(update_message, "Downloading...");
    lv_label_set_long_mode(update_message, LV_LABEL_LONG_WRAP);
    lv_obj_set_width(update_message, LV_HOR_RES - 20);
    lv_obj_set_style_text_font(update_message, &lv_font_montserrat_14, LV_PART_MAIN);
    lv_obj_set_style_text_color(update_message, lv_color_hex(WHITE), LV_PART_MAIN);
    lv_obj_set_style_text_align(update_message, LV_TEXT_ALIGN_CENTER, LV_PART_MAIN);
    lv_obj_set_style_text_line_space(update_message, 4, LV_PART_MAIN);
...
void update_ota_state(ota_state_t state) {
    if (lvgl_lock(pdMS_TO_TICKS(portMAX_DELAY), __FILE__, __LINE__)) {
        ...
        char version_str[32];
        snprintf(version_str, sizeof(version_str), "%d.%d.%d", state.latest_version.major, state.latest_version.minor,
                 state.latest_version.patch);
        lv_label_set_text_fmt(version_label, "Version: %s", version_str);
        ...
        lv_label_set_text_fmt(progress_label, "%.2f%%\n(%s/%s)", percent, received, total);
