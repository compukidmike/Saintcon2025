...
/**
 * @brief Register a handler for D-pad events.
 *
 * If the minibadge event loop is not yet created the handler will be stored and automatically
 * registered once minibadge_init() creates the loop. Multiple handlers may be registered up to a
 * small fixed limit.
 *
 * @param handler The event handler function
 * @param arg     User argument passed to the handler when invoked
 * @return ESP_OK on success, ESP_ERR_NO_MEM if capacity exceeded, or esp_event error code
 */
...
/**
 * @brief Query whether the D-pad polling task is currently active.
 */
