...
esp_err_t se_init() {
    ...
    // Try initializing the ATECC device with a few retries. Some boards
    // have the SE powering up slowly or the I2C bus may not be fully
    // ready immediately at boot; retrying helps make init more robust.
    ...
    for (int attempt = 0; attempt < init_retries; ++attempt) {
        ...
        ESP_LOGW(TAG, "atcab_init attempt %d/%d failed: %d - retrying...", attempt + 1, init_retries, status);
...
ATCA_STATUS io_key_seed(uint8_t seed_out[32]) {
    ...
    if (data_locked) {
        // Retry I2C read operations to handle transient bus contention
        // Use more retries and longer delays since bus can be busy during boot
        const int max_retries       = 10;
        const int retry_delays_ms[] = {10, 20, 50, 100, 150, 200, 300, 400, 500, 750};
        bool read_success           = false;

        for (int retry = 0; retry < max_retries; retry++) {
            status = atcab_read_bytes_zone(ATCA_ZONE_DATA, SE_KEY_IO_SEED, 0, masked, sizeof(masked));
            ...
            if (status == ATCA_SUCCESS) {
                ...
                if (retry > 0) {
                    ESP_LOGI(TAG, "Retrieved I/O key seed from slot %d after %d retries", SE_KEY_IO_SEED, retry);
                } else {
            ...
            // Retry on most errors except specific non-transient ones
            // Non-transient errors that should NOT retry:
            // - ATCA_BAD_PARAM (-30): Invalid parameters
            // - ATCA_INVALID_ID (-29): Device not initialized
            // - ATCA_INVALID_SIZE (-28): Size mismatch
            // Everything else (including ATCA_GEN_FAIL, ATCA_RX_FAIL, ATCA_TX_FAIL, ATCA_COMM_FAIL,
            // ATCA_RX_CRC_ERROR, ATCA_RX_TIMEOUT, ATCA_TX_TIMEOUT, etc.) should be retried
            bool should_retry = (status != ATCA_BAD_PARAM && status != ATCA_INVALID_ID && status != ATCA_INVALID_SIZE);

            if (should_retry) {
                if (retry < max_retries - 1) {
                    ESP_LOGD(TAG, "I2C read failed with error %d, retrying in %dms (attempt %d/%d)", status,
                             retry_delays_ms[retry], retry + 1, max_retries);
                    vTaskDelay(pdMS_TO_TICKS(retry_delays_ms[retry]));
                }
            } else {
                // Non-transient error (e.g., bad parameters) - don't retry
                ESP_LOGD(TAG, "Non-retriable error reading slot %d: %d, falling back to NVS", SE_KEY_IO_SEED, status);
                break;
            }
        }

        if (!read_success) {
            ESP_LOGW(TAG, "Could not read I/O key seed from slot %d after %d attempts: %d", SE_KEY_IO_SEED, max_retries, status);
        }
    }
...
esp_err_t se_read_slot(se_slot_id_t slot, uint8_t *value, size_t len) {
    ...
    if (((config->SlotConfig[slot] & ATCA_SLOT_CONFIG_ENC_READ_MASK) != 0) &&
        ...
        for (size_t i = 0; i < len; i += 32) {
            ...
            for (int retry = 0; retry < max_retries; retry++) {
                status = atcab_read_enc(slot, block, data, io_key, SE_KEY_IO_KEY, num_in);

                if (status == ATCA_SUCCESS) {
                    if (retry > 0) {
                        ESP_LOGD(TAG, "Read slot %d block %d succeeded after %d retries", slot, block, retry);
                    } else {
            ...
            if (!read_success) {
                ESP_LOGE(TAG, "Failed to read from slot %d block %d after %d attempts (offset %zu, len %zu): %d", slot, block,
                         max_retries, i, rlen, status);
                scrubmem(io_key, sizeof(io_key));
                CRYPTOAUTH_UNLOCK();
                return ESP_FAIL;
            }
    ...
    // Plaintext reads
        ...
        for (int retry = 0; retry < max_retries; retry++) {
            status = atcab_read_bytes_zone(ATCA_ZONE_DATA, slot, 0, value, len);

            if (status == ATCA_SUCCESS) {
                if (retry > 0) {
                    ESP_LOGD(TAG, "Read from slot %d succeeded after %d retries", slot, retry);
                }
                read_success = true;
                break;
            }
        ...
        if (!read_success) {
            ESP_LOGE(TAG, "Failed to read from slot %d after %d attempts: %d", slot, max_retries, status);
            CRYPTOAUTH_UNLOCK();
            return ESP_FAIL;
        }
    } else {
