...
static void ota_task(void *arg) {
    while (true) {
        ...
        // Reset the OTA state
        ota_state.current_version = firmware_version();
        ota_state.latest_version  = (version_t){0};
        ...
        api_result_t *firmware_result = api_get_firmware_head();
        if (firmware_result == NULL) {
            ...
            api_firmware_head_t *data = (api_firmware_head_t *)firmware_result->data;
            snprintf(latest_version, sizeof(latest_version), "%s", data->firmware_version);
        ...
        if (firmware_version_compare(latest_version) >= 0) {
            ...
            version_parse(latest_version, &ota_state.latest_version);
        }
