...
static touch_data_t data_buffer;
    ...
    #if CONFIG_INPUT_TOUCH_USE_INTERRUPT
...
void touch_read_task(void *_arg) {
    ...
    while (1) {
        if (xSemaphoreTake(data_avail, portMAX_DELAY) == pdTRUE) {
            ...
            if (now - last_touch_time < pdMS_TO_TICKS(CONFIG_INPUT_TOUCH_DEBOUNCE_MS)) {
                continue;
            }
...
void init_lcd_touch() {
    ...
    esp_lcd_touch_config_t tp_config = {
        .x_max = params.v_res,
        .y_max = params.h_res,
    #ifdef CONFIG_INPUT_TOUCH_RST_GPIO
        .rst_gpio_num = CONFIG_INPUT_TOUCH_RST_GPIO,
    #else
        .rst_gpio_num = GPIO_NUM_NC,
    #endif
    #ifdef CONFIG_INPUT_TOUCH_INT_GPIO
        .int_gpio_num = CONFIG_INPUT_TOUCH_INT_GPIO,
    #else
    ...
    #if CONFIG_INPUT_TOUCH_CONTROLLER_GT911
    ...
    return;
    #endif // CONFIG_INPUT_TOUCH_CONTROLLER_GT911
